...
class Preprocessor:
    ...
    def map_atom_info(df, structures, index):
        df = pd.merge(df, structures, how='left', left_on=['molecule_name', f'atom_index_{index}'], right_on=['molecule_name', 'atom_index'])
        df = df.drop(columns='atom_index')
        df = df.rename(columns={'atom': f'atom_{index}', 'x': f'x_{index}', 'y': f'y_{index}', 'z': f'z_{index}'})
        return df
